...
### Отказоустойчивость и персистентность данных

Одной из главных причин использования оркестраторов, таких как Docker Swarm, является обеспечение высокой доступности приложений. Swarm отлично справляется с перезапуском упавших контейнеров, но что происходит с данными, если сервис требует их сохранения (является stateful)? Это один из самых важных и сложных вопросов в работе с кластеризованными системами.

В этом разделе мы на практических примерах разберем, как Swarm ведет себя при сбое узла, почему это создает проблему для stateful-сервисов и как эту проблему решить.

#### Поведение Swarm по умолчанию: перезапуск без сохранения состояния

Представим, что у нас есть сервис, который пишет важные данные в файл (например, логи, пользовательские загрузки или состояние сессии). Когда мы развертываем такой сервис в Swarm, мы обычно используем тома (`volumes`) для хранения этих данных вне контейнера.

По умолчанию Docker создает тома в локальном хранилище узла, на котором запущен контейнер.

**Что происходит при сбое узла?**

1.  **Обнаружение сбоя**: Менеджер Swarm обнаруживает, что один из узлов перестал отвечать.
2.  **Пересоздание задач**: Swarm немедленно принимает меры для восстановления желаемого состояния. Он создает новые задачи (контейнеры) для сервисов, которые работали на упавшем узле, и запускает их на здоровых рабочих узлах.
3.  **Потеря данных**: Новый контейнер, запущенный на новом узле, создает **новый пустой том** в локальном хранилище *этого нового узла*. Данные, которые были в томе на упавшем узле, остаются там и **не переносятся автоматически**.

**Итог:** Ваше приложение снова работает, но оно потеряло все данные, которые были сгенерированы на старом узле. Для stateless-приложений (которые не хранят состояние, например, большинство веб-серверов) это не проблема. Но для stateful-приложений (базы данных, системы кэширования, приложения с загрузкой файлов) это катастрофа.

#### Упражнение 1: Демонстрация проблемы

Давайте увидим эту проблему в действии. Мы создадим сервис, который каждую секунду записывает текущую дату в файл `/data/time.log`.
...
docker service create \
  ...
  --mount type=volume,source=my-data,destination=/data \
  alpine sh -c "while true; do date >> /data/time.log; sleep 1; done"
```

Разберем команду:
- `--name stateful-writer`: Имя нашего сервиса.
- `--mount type=volume,source=my-data,destination=/data`: Мы создаем и монтируем том с именем `my-data` в папку `/data` внутри контейнера. Так как мы не указали драйвер, это будет стандартный локальный том.
- `alpine sh -c "..."`: Мы используем легкий образ `alpine` и запускаем в нем бесконечный цикл, который дописывает дату в файл `/data/time.log`.

**Шаг 2: Проверка записи данных**

Сначала найдем, на каком узле запущен наш контейнер:
```shell
docker service ps stateful-writer
```
Вы увидите что-то вроде этого (нас интересует колонка `NODE`):
```
ID             NAME                  IMAGE          NODE      DESIRED STATE   CURRENT STATE          ERROR     PORTS
p8a...         stateful-writer.1     alpine:latest  worker1   Running         Running 22 seconds ago
```
В данном случае, наш контейнер работает на узле `worker1`.

Теперь давайте подключимся к контейнеру и посмотрим на наш файл. Вам нужно будет выполнить `docker exec` на том узле, где запущен контейнер (`worker1` в нашем примере).

```shell
# На узле worker1
docker exec -it $(docker ps -q --filter "name=stateful-writer") cat /data/time.log
```
Вы должны увидеть несколько строк с датой и временем, что подтверждает, что данные успешно записываются. Повторите команду через несколько секунд, и вы увидите новые строки.

**Шаг 3: Симуляция сбоя узла**

Вернемся на **менеджер-узел**. Давайте "выключим" узел `worker1`, переведя его в режим `drain`. Этот режим остановит все контейнеры на узле и сделает его недоступным для новых задач.
...
**Шаг 4: Проверка восстановления и потери данных**

Подождите несколько секунд и снова проверьте состояние сервиса:
```shell
docker service ps stateful-writer
```
Вы увидите интересную картину:
```
ID             NAME                  IMAGE          NODE      DESIRED STATE   CURRENT STATE                ERROR     PORTS
q9b...         stateful-writer.1     alpine:latest  worker2   Running         Running 5 seconds ago
p8a...         stateful-writer.1     alpine:latest  worker1   Shutdown        Shutdown 15 seconds ago
```
Swarm сделал свою работу: он обнаружил, что задача на `worker1` больше не может выполняться, остановил ее (`Shutdown`) и запустил новую задачу на доступном узле `worker2`. Приложение снова онлайн!

Но что стало с нашими данными? Давайте проверим файл в новом контейнере. Подключитесь к узлу `worker2` и выполните ту же команду `docker exec`:

```shell
# На узле worker2
docker exec -it $(docker ps -q --filter "name=stateful-writer") cat /data/time.log
```
Вы увидите, что файл содержит всего несколько строк, которые были записаны с момента запуска *нового* контейнера. Все данные, которые были сгенерированы на узле `worker1`, **исчезли**.

Это упражнение наглядно демонстрирует, почему стандартные локальные тома не подходят для stateful-сервисов в Docker Swarm.

#### Решение: использование общего хранилища (Shared Storage)

Проблема, которую мы увидели, решается путем использования **общего хранилища**. Идея проста: вместо того чтобы хранить данные на локальном диске каждого узла, мы используем сетевое хранилище, которое доступно для всех узлов в кластере.

Когда контейнер создает том, он фактически будет использовать это общее хранилище. Таким образом, если контейнер "переезжает" с одного узла на другой, новый контейнер просто подключается к тому же самому общему хранилищу и продолжает работу с того же места, где остановился предыдущий. Данные больше не теряются.

Существует множество технологий для организации общего хранилища. Вот некоторые из них:

-   **NFS (Network File System)**: Простой и широко распространенный протокол для предоставления общего доступа к файлам по сети. Его легко настроить, что делает его отличным выбором для обучения и для многих реальных сценариев.
-   **GlusterFS / Ceph**: Это более сложные, но и более мощные распределенные файловые системы. Они обеспечивают высокую производительность и отказоустойчивость самого хранилища, реплицируя данные между несколькими серверами.
-   **Облачные хранилища**: Если ваш кластер развернут в облаке (AWS, Google Cloud, Azure), вы можете использовать их нативные решения для блочных хранилищ (например, Amazon EBS, Google Persistent Disk), которые можно монтировать к вашим узлам.

В следующем упражнении мы настроим простое общее хранилище с помощью NFS и убедимся, что оно решает нашу проблему с потерей данных.

#### Упражнение 2: Решение проблемы с помощью NFS

Теперь давайте решим проблему, используя NFS (Network File System) в качестве нашего общего хранилища. Мы настроим NFS-сервер на нашем менеджер-узле, а рабочие узлы будут клиентами.

**Шаг 1: Настройка NFS-сервера (на менеджер-узле)**

*Эти команды предназначены для Ubuntu/Debian. Для других ОС могут потребоваться другие пакеты.*
...
3.  **Настройте экспорт папки.** Откройте файл `/etc/exports` для редактирования:
    ```shell
    sudo nano /etc/exports
    ```
    Добавьте в конец файла следующую строку. Она разрешает доступ к папке `/mnt/nfs_share` с любого IP-адреса (`*`). В реальной среде здесь следует указать IP-адреса ваших рабочих узлов.
    ```
...
**Шаг 2: Настройка NFS-клиентов (на всех рабочих узлах)**

Теперь на **каждом рабочем узле** установите клиентский пакет NFS.
...
**Шаг 3: Развертывание сервиса с NFS-томом**

Вернитесь на **менеджер-узел**. Удалите старый сервис, если вы этого еще не сделали:
```shell
docker service rm stateful-writer
```

Теперь создадим новый сервис. Команда будет похожа на предыдущую, но с очень важными изменениями в флаге `--mount`.

```shell
# Замените <MANAGER-IP> на реальный IP-адрес вашего менеджер-узла
docker service create \
  --name stateful-writer-nfs \
  --mount type=volume,destination=/data,volume-driver=local,volume-opt=type=nfs,volume-opt=o="addr=<MANAGER-IP>,rw",volume-opt=device=":/mnt/nfs_share" \
  alpine sh -c "while true; do date >> /data/time.log; sleep 1; done"
```

Давайте внимательно разберем опции `--mount`:
- `type=volume,destination=/data`: Как и раньше, монтируем том в `/data`.
- `volume-driver=local`: Это может показаться странным, но мы используем встроенный драйвер `local`, который умеет работать с NFS.
- `volume-opt=type=nfs`: Указываем, что тип файловой системы для тома — NFS.
- `volume-opt=o="addr=<MANAGER-IP>,rw"`: Самая важная часть. В опциях (`o`) мы передаем адрес нашего NFS-сервера (`addr=<MANAGER-IP>`) и права на чтение/запись (`rw`).
- `volume-opt=device=":/mnt/nfs_share"`: Указываем, какую именно папку на NFS-сервере мы хотим смонтировать.

**Шаг 4: Повторение симуляции сбоя**

Процесс точно такой же, как в предыдущем упражнении.

1.  **Найдите, на каком узле запущен сервис** (`docker service ps stateful-writer-nfs`). Допустим, это снова `worker1`.
2.  **Проверьте данные.** Подключитесь к `worker1` и выполните `docker exec ... cat /data/time.log`. Убедитесь, что данные пишутся.
3.  **"Уроните" узел.** На менеджер-узле выполните `docker node update --availability drain worker1`.
4.  **Проверьте состояние сервиса.** Выполните `docker service ps stateful-writer-nfs` и убедитесь, что контейнер был перезапущен на `worker2`.
5.  **ПРОВЕРКА РЕШЕНИЯ.** Теперь самое главное. Подключитесь к узлу `worker2` и проверьте файл логов в новом контейнере:
    ```shell
    # На узле worker2
    docker exec -it $(docker ps -q --filter "name=stateful-writer-nfs") cat /data/time.log
    ```

В этот раз вы увидите, что файл содержит **все строки**, включая те, что были записаны, когда контейнер работал на `worker1`. Данные не были потеряны! Новый контейнер просто "подхватил" тот же самый сетевой том.

**Шаг 5: Очистка**

Не забудьте вернуть узел в рабочее состояние и удалить сервис.
```shell
# На менеджер-узле
docker node update --availability active worker1
docker service rm stateful-writer-nfs
```

Это упражнение показывает, как правильная настройка хранилища обеспечивает настоящую отказоустойчивость для stateful-приложений в Docker Swarm.

Этот пример с NFS — мощная демонстрация того, как Docker Swarm в сочетании с правильными технологиями хранения данных позволяет создавать по-настоящему отказоустойчивые системы.
