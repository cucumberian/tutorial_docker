...
Любая команда в докерфайле создает новый слой имеджа. Поэтому иногда объединяют команды, чтобы уменьшить количество слоев.
...
`docker run --name hello -d hello-world` - запуск контейнера с именем(--name) hello из имеджа (hello-world) в фоне (-d)
`docker stop <имя или id>` - остановить контейнер c заданным именем или id
`docker run --rm hello-world` - запуск контейнера из имеджа, который удалится как отработает или будет остановлен
